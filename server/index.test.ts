...
import { AiProviderError, parseExplanationResponse } from "./services/aiService.js";
...
describe("CodeCoach API", () => {
  ...
  it("returns 429 for Gemini rate limits", async () => { const failingService: AiService = { ...service, generateHint: vi.fn().mockRejectedValue(new AiProviderError("rate-limit", "busy", 429)) }; const response = await request(createApp(failingService)).post("/api/hint").send({ challengeId: "two-sum", code: "return [];", hintLevel: 1, language: "JavaScript" }); expect(response.status).toBe(429); expect(response.body).toEqual({ error: "AI service is temporarily busy. Please try again." }); });
  it("rejects malformed explanation requests", async () => { const response = await request(createApp(service)).post("/api/explain").send({ challengeId: "two-sum", code: "", language: "JavaScript" }); expect(response.status).toBe(400); });
  it("rejects malformed structured explanation output", () => { expect(() => parseExplanationResponse('{"summary":"missing required fields"}')).toThrow(); });
  it("serves repeated hint requests through the same API contract", async () => { const response = await request(createApp(service)).post("/api/hint").send({ challengeId: "two-sum", code: "return [];", hintLevel: 2, language: "JavaScript" }); const secondResponse = await request(createApp(service)).post("/api/hint").send({ challengeId: "two-sum", code: "return [];", hintLevel: 3, language: "JavaScript" }); expect(response.status).toBe(200); expect(secondResponse.status).toBe(200); expect(service.generateHint).toHaveBeenCalledTimes(3); });
